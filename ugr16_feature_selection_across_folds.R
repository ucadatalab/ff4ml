...
if (TYPE == "ts") {
        ...
        folds   = read.csv(file=paste0("./data/ugr16/dat_", TYPE, "/ugr16_multiclass_folds_", NUMREPS, "x", NUMFOLDS, "cv.csv"), header=T, sep=",")
        outfile.name = paste0("./data/ugr16/dat_", TYPE, "/ugr16_multiclass_folds_selecvars_", NUMREPS, "x", NUMFOLDS, "cv.csv")
        X = dataset[,2:135]
} else {
        dataset = read.csv(file=paste0("./data/ugr16/dat_", TYPE, "/ugr16_all_extended_", NUMOBS, "fobs_multiclass.csv"), header=T, sep=",")
        folds   = read.csv(file=paste0("./data/ugr16/dat_", TYPE, "/ugr16_all_extended_", NUMOBS, "fobs_multiclass_folds_", NUMREPS, "x", NUMFOLDS, "cv.csv"), header=T, sep=",")
        outfile.name = paste0("./data/ugr16/dat_", TYPE, "/ugr16_all_extended_", NUMOBS, "fobs_multiclass_folds_selecvars_", NUMREPS, "x", NUMFOLDS, "cv.csv")
        X = dataset[,1:134]
}
...
for (rep in 1:ncol(folds)) {
        for (caja in 1:max(folds[,1])) {
                X.train = X[folds[,rep] != caja,]
                y.train = y[folds[,rep] != caja]
                ...
                cat("Repeticion", rep, "; Caja", caja, "; Seleccionadas", paste0(length(selected.variables), "/", ncol(X.train)), "\n")
                
                row.vars.selected = rep(0, ncol(X.train))
                names(row.vars.selected) = colnames(X.train)
                row.vars.selected[selected.variables] = 1
                
                to.save = c(rep, caja, row.vars.selected)
